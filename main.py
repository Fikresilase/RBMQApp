...
class ImageProcessor(QMainWindow):
    ...
    def process_reversion(self):
        """Process reversion from packed data with step-by-step display."""
        if not self.image_path or not self.image_path.endswith('.bin'):
            QMessageBox.warning(self, "Error", "Please select a .bin file first!")
            return
        
        try:
            print("\n=== Starting Bit Reduction Reversion Process ===")
            ...
            # For this example, we'll assume grayscale (single channel)
            # In a real app, you'd need to store the original shape and pad_length
            # For now, we'll use hardcoded values or prompt the user
            height = int(input("Enter original image height: "))
            width = int(input("Enter original image width: "))
            self.original_shape = (height, width)
            self.pad_length = 0  # Assume no padding for now
            
            # 2. Unpack to 5-bit values
            print("\nStep 2: Unpacking to 5-bit values...")
            five_bit = unpack_bytes_to_5bit(packed_data, self.original_shape, self.pad_length)
            
            # Display first 10 5-bit values
            print("\n5-bit values (first 10 pixels):")
            for i in range(min(10, five_bit.size)):
                val = five_bit.flat[i]
                print(f"Pixel {i}: {val} (binary: {val:05b})")
            
            # 3. Remap to median values
            print("\nStep 3: Remapping to median values...")
            restored = np.vectorize(INDEX_TO_MEDIAN.get)(five_bit)
            
            # Display first 10 remapped values
            print("\nRemapped 8-bit values (first 10 pixels):")
            for i in range(min(10, restored.size)):
                val = restored.flat[i]
                print(f"Pixel {i}: {val} (binary: {val:08b})")
            
            # 4. Create and display image
            print("\nStep 4: Creating image from remapped values...")
            self.processed_img = Image.fromarray(restored.astype(np.uint8))
            ...
            print(f"\n!!! Error in reversion process: {str(e)}")
            QMessageBox.critical(self, "Error", f"An error occurred during reversion:\n{str(e)}")
